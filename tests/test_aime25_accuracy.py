...
"""
Test AIME 2025 accuracy with optimized retriever v2
...
CACHE_DIR = "./cache_aime25"
MAX_OPENAI_CONC = 10
OUTPUT_FILE = "./aime25_results.json"
...
class StrategyRetriever:
    ...
    def retrieve_strategies(self, problem_text: str, k: int = 5, subject: str = None, preferred_source: str = 'auto') -> List[Dict]:
        ...
        return self.retriever.retrieve(
            ...
            ann_top=50,
            preferred_source=preferred_source  # 可配置的source选择
        )
                    ...
                    for retry_num in range(1, max_retries + 1):
                        ...
                        async with session.post(
                            ...
                            if retry_resp.status == 200:
                                ...
                                reasoning_content = retry_message.get('reasoning_content') or ''
                                content = retry_message.get('content') or ''
                                if content:
...
async def main():
    print("="*70)
    print("AIME 2025 Accuracy Test with Optimized Retriever V2")
    print("="*70)

    # 1. Load AIME 2025 from HuggingFace
    print(f"\n[1/5] Loading AIME 2025 from HuggingFace (math-ai/aime25)...")
    ...
    try:
        ...
        for item in dataset:
            problems.append({
                ...
                'answer': item['answer'],
                'problem_id': f"AIME25_{len(problems)+1}"
            })
    ...
    for i, prob in enumerate(problems):
        ...
        strategies = retriever.retrieve_strategies(prob['problem'], k=7)
