#!/usr/bin/env python3
"""
Build Knowledge Graph and Train GNN
...
def main():
    print("="*70)
    print("BUILDING STRATEGY KNOWLEDGE GRAPH")
    print("="*70)
    ...
    # Load full strategy dataset (4850 problems)
    # This script supports BOTH data formats:
    # - Old format (hybrid_kg_system): Strategies as dicts with metadata
    # - New format (huggingface_math_dataset_v2): Strategies as simple strings
    kg.load_data_from_final_dataset(
        dataset_path='data/strategy_dataset_full.json',
        human_path=None,  # Optional: path to human strategy preference file
        model_path=None   # Optional: path to model strategy preference file
    )
    ...
    with open('strategy_kg.pkl', 'wb') as f:
        pickle.dump(kg, f)
    print("✓ Saved knowledge graph: strategy_kg.pkl")

    # Save GNN model
    torch.save(gnn_model.state_dict(), 'gnn_model.pth')
    print("✓ Saved GNN model: gnn_model.pth")

    # Save embeddings
    for name, emb in embeddings.items():
        np.save(f'{name}_emb.npy', emb)
        print(f"✓ Saved {name}_emb.npy")

    # Save classifier
    with open('strategy_classifier.pkl', 'wb') as f:
        pickle.dump(classifier, f)
    print("✓ Saved classifier: strategy_classifier.pkl")

    print("\n" + "="*70)
    print("✅ KNOWLEDGE GRAPH BUILD COMPLETE!")
    print("="*70)
    print("\nGenerated files:")
    print("  - strategy_kg.pkl (knowledge graph)")
    print("  - gnn_model.pth (trained GNN)")
    print("  - *_emb.npy (6 embedding files)")
    print("  - strategy_classifier.pkl (classifier)")
